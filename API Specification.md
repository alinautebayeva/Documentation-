...
| --- | --- |
| Автор             | Алина                                                                                                                                                             |
| Задача            | Реализовать возможность бронировать переговорную комнату через мобильное приложение MeetRoom                                                                               |
| Бизнес-постановка | Пользователь (сотрудник компании) бронирует свободную переговорную комнату в приложении. Решение автоматизирует процесс бронирования и заменяет текущую практику согласования через электронную почту, снижая нагрузку на сотрудников и минимизируя риски ошибок и конфликтов по времени.
...
Авторизация запросов осуществляется с использованием токена доступа (accessToken). После успешной авторизации пользователь получает accessToken, который должен передаваться в заголовке HTTP в формате Bearer <token>. После истечения срока действия accessToken клиент обновляет его с помощью refreshToken. Срок действия access token: 15 минут, срок действия refresh token: 30 дней.
...
```
{
  "accessToken": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.aWQ6MTIzNDUscm9sZT11c2Vy.exp1707483600.signatureX9F3kL2",
  "refreshToken": "rfr_9cA7LQ2xKpM4ZVdE8N1JYH0sT6WmB5R",
  "user": {
    "userID": 12345,
    "email": "ivanpetrov@example.com",
    "firstname": "Иван",
...
|---------------|---------|------------------------------------------------------------------------|
| accessToken   | string  | Авторизация запросов к API                                           |
| resfreshToken | string  | Обновление access token без повторного логина                         |
...
Тело ответа, если не введен email:
...
```
Тело ответа, если не введен пароль: 
```
{
...
| start_date | string (YYYY-MM-DD) | необязательный              | Начальная дата для фильтра по периоду времени. Значение start_date не может быть больше значения end_date |
...
| roomID  | integer           | Идентификатор переговорной комнаты                                                                  |
| roomName  | string           | Название переговорной комнаты                                                                     |
| floor         | integer | Этаж, на котором находится переговорная |
| start_time | string (datetime) | Дата и время старта бронирования. (Формат ISO 8601). |
| end_time   | string (datetime) | Дата и время окончания бронирования (Формат ISO 8601)                                                        |
| status   | string  |  Статус бронирования (active/cancelled)                                                        |
...
Ошибка выдается в случае, если start_date > end_date.

Тело ответа: 

```
{
"message": "Bad Request: Invalid start_date or end_date."
}
```
...
## 3.4 Проверка доступности выбранной переговорной комнаты
Метод предназначен для проверки, свободна ли выбранная переговорная комната в указанный временной интервал. Переговорная комната считается свободной, если в указанный интервал времени нет активных бронирований, пересекающихся с заданным диапазоном. При проверке учитываются только бронирования со статусом active, бронирования со статусом canceled игнорируются.

__Метод:__ GET <br>
__URL:__ api/rooms/{roomID} <br>
__Content-Type:__ application/json <br>
Authorization: Bearer <accessToken>
...
```
GET /api/rooms/{roomID}?start_time=2026-02-10T14:00:00&end_time=2026-02-10T15:00:00
```
...
| Параметр      | Тип     | Обязательный/необязательный | Описание                                |
| --- | --- | --- | --- |
 start_time | string (datetime) | обязательный  | Время старта бронирования (Формат ISO 8601) Значение start_time не может быть больше значения end_time
 end_time | string (datetime) | обязательный  | Время окончания бронирования (Формат ISO 8601) 

### __Ответы:__ 

#### 1. 200 OK (Комната свободна) 
...
```
{
  ...
  "isAvailable": false,
  "conflictingBooking": {
    "start_time": "2026-02-10T14:30:00",
    "end_time": "2026-02-10T15:30:00"
  }
}
```
...
| --- | --- | --- |
| roomID   | integer | Идентификатор переговорной комнаты |
| isAvailable | boolean | true - комната свободна, false - занята     |
| conflictingBooking | object | Объект с информацией о пересекающемся бронировании. Возвращается только если isAvailable = false    |
...
Ошибка выдается, в случае если переговорной комнаты с таким значением roomID не существует. 
...
## 3.5 Создание бронирования переговорной комнаты 
...
| roomID  | integer           | Идентификатор переговорной комнаты                                                                  |
| roomName  | string           | Название переговорной комнаты                                                                     |
| capacity      | integer | Максимальная вместимость переговорной   |
...
## 3.6 Изменение времени бронирования  

Метод позволяет изменить время старта и время окончания бронирования.

__Метод:__ PUT <br>
__URL:__ api/bookings/{bookingid} <br>
__Content-Type:__ application/json <br>
Authorization: Bearer <accessToken>
...
Параметр | Тип | Обязательный/необязательный | Описание  
---|---|---|---
 start_time | string (datetime) | обязательный  | Новое время старта бронирования (Формат ISO 8601) Значение start_time не может быть больше значения end_time 
...
| end_time   | string (datetime) | Дата и время окончания бронирования (Формат ISO 8601)                                                        |
| status   | string  |  Статус бронирования (active/cancelled)                                                        |
| changed_at | string (datetime) | Дата и время внесения изменений (Формат ISO 8601)                                                           |
